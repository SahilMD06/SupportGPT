...
import { Send, Bot, User, Copy, Check, ArrowUp, Sparkles } from 'lucide-react';
...
const SUGGESTIONS = [
  ...
  'I\'ve been waiting 2 weeks and need this resolved urgently',
];
    ...
    <div className="flex flex-col items-center justify-center h-full px-6 text-center">
      ...
      >
        ...
        <p className="text-sm max-w-xs" style={{ color: 'var(--text-tertiary)' }}>
          Ask anything — our AI automatically routes your question to the right specialist.
        </p>
      </motion.div>
...
function ChatInner() {
  ...
  useEffect(() => {
    if (sessionParam && sessionParam !== currentSessionId) loadSession(sessionParam);
  }, [sessionParam]);

  const loadSession = async (sid: string) => {
    try {
      const res = await historyAPI.getConversation(sid);
      const { messages: msgs, session_id } = res.data;
  ...
  const handleSend = useCallback(async (text?: string) => {
    ...
    try {
          ...
          try {
            ...
            if (data.type === 'session') { setCurrentSession(data.session_id); }
              ...
              if (last?.role === 'assistant') {
                updated[updated.length - 1] = { ...last, agents_used: agents, intents, sources, isStreaming: false };
      ...
      try {
        ...
        const { response, session_id, intents, agents_used, sources } = res.data;
        setCurrentSession(session_id);
        addMessage({ id: `a-${Date.now()}`, role: 'assistant', content: response, timestamp: new Date().toISOString(), agents_used, intents, sources });
          ...
          <div className="max-w-2xl mx-auto">
            ...
            >
              <textarea
                ...
                onKeyDown={onKeyDown}
                placeholder="Ask about billing, technical issues, product features..."
                rows={1}
